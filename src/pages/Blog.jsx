...
import { FiCalendar, FiUser, FiTag, FiSearch, FiArrowRight } from 'react-icons/fi';
  ...
  useEffect(() => {
    const fetchBlogPosts = async () => {
      try {
        ...
        const response = await fetch('https://thecosmicstackk.onrender.com/blogs');
        setBlogPosts(response.data);
        setError(null);
      } catch (err) {
        console.error('Error fetching blog posts:', err);
        setError('Failed to load blog posts. Please try again later.');
        // You can use mock data as fallback if API fails
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <FadeIn className="text-center mb-16">
            ...
            <p className="text-slate-400 text-lg md:text-xl max-w-3xl mx-auto">
              Explore our latest thoughts, insights, and expertise on web development, design, and technology.
            </p>
          </FadeIn>
        ...
        <div className="relative h-48 mb-6 overflow-hidden rounded-lg">
          <div className="absolute inset-0 bg-gradient-to-br from-blue-500/20 to-purple-500/20 flex items-center justify-center">
            ...
            <div className="absolute inset-0 flex items-center justify-center px-4">
              <h3 className="text-lg font-bold text-white text-center">{post.title}</h3>
            </div>
          </div>
        </div>
      );
    ...
    >
      ...
      <div className="flex flex-wrap gap-2 mb-6">
        {post.tags && post.tags.map(tag => (
          <span key={tag} className="text-xs text-slate-400 border border-slate-700 rounded-full px-2 py-1">
            #{tag}
          </span>
        ))}
      </div>
